...
//Initialization values for lighting
...
GLint Shine =128;
GLint walkX=1,walkY=0,lookX=0,lookY=0;
...
void eyebrowleft()
{
    ...
    glRotatef(180,1,0,0);
    glColor3f(0.0,0.0,0.0);
    gluCylinder(gluNewQuadric(),.06,.01,.6,4,6);
    glPopMatrix();
}
...
void eyebrowright()
{
    ...
    glRotatef(-45,0,1,0);
    glRotatef(-180,1,0,0);
    gluCylinder(gluNewQuadric(),.06,.01,.4,4,6);
    glPopMatrix();
}
